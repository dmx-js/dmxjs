...
export async function autoconfig(config: AutoconfigOptions = {}) {
  ...
  const autoconfig = await promiseWithTimeout(
    timeout,
    network.autoconfigFromPeers()
  );

  if (autoconfig.state === "timeout") {
    throw new AutoconfigFailureError(
      "Autoconfig timed out waiting for peers to connect"
    );
  }
  ...
  if (!network.isConnected()) {
    throw new AutoconfigFailureError(
      "Autoconfig failed to connect to the network"
    );
  }
